        ...
        dplyr::mutate(dt_created = as.POSIXct(x = dt_created,
                      ...
                      dt_end     = as.POSIXct(x = dt_end,
                                              tryFormats = c("%Y-%m-%dT%H:%MZ",
                                                             "%Y-%m-%dT%H:%M:%SZ"),
                                              tz = "UTC")) %>%
        merge(y     = entsoeapi::StandardProcessTypeList[, c("CODE", "DEFINITION")] %>%
                dplyr::rename(process_type     = CODE,
                              process_type_def = DEFINITION),
              by    = "process_type",
              all.x = TRUE) %>%
        merge(y     = entsoeapi::StandardBusinessTypeList[, c("CODE", "DEFINITION")] %>%
                dplyr::rename(business_type     = CODE,
                              business_type_def = DEFINITION),
              by    = "business_type",
              all.x = TRUE) %>%
        merge(y     = entsoeapi::StandardObjectAggregationTypeList[, c("CODE", "DEFINITION")] %>%
                dplyr::rename(object_aggregation     = CODE,
                              object_aggregation_def = DEFINITION),
              by    = "object_aggregation",
              all.x = TRUE) %>%
        merge(y     = entsoeapi::StandardCurveTypeList[, c("CODE", "DEFINITION")] %>%
                dplyr::rename(curve_type     = CODE,
                              curve_type_def = DEFINITION),
              by    = "curve_type",
              all.x = TRUE) %>%
        merge(y     = entsoeapi::StandardAssetTypeList[, c("CODE", "DEFINITION")] %>%
