...
16. The lookup table `.eic_chars` maps the 37 allowed characters (`0–9`, `A–Z`,
`-`) to integers 0–36. `assert_eic()` computes a weighted sum of the first 15
characters (weights 16 down to 2), derives the expected check character via
`(36 - (sum - 1) %% 37) + 1`, and returns `FALSE` if it does not match the
actual 16th character. This function is not exported; it is called internally
immediately after each `checkmate::assert_string()` EIC check.
...
1. **URL construction.** Assembles the full URL from hard-coded constants:
   - Scheme: `https://`
