...
A one-liner that wraps `api_req()` with `purrr::safely()`. All R-level
exceptions are caught and returned as `list(result = NULL, error = <condition>)`
rather than halting execution. This standardised return shape is what
...
3. **Execution.** Sent via `purrr::safely(httr2::req_perform)` — another safety
...
`purrr::safely(utils::unzip)`, then reads each extracted XML file with
...
- If `$result` is a list (paginated or zipped): iterates with `purrr::imap()`,
  ...
  combines all results with `data.table::rbindlist()` and converts to a tibble.
...
After enrichment, `xml_to_table()` applies a hard-coded whitelist of ~90 allowed
